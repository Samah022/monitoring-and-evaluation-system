...
class PreprocessingFrame:
    def __init__(self, resize_scale):
        self.__resize_scale = self.set_resize_scale(resize_scale)
    ...
    def set_resize_scale(self, resize_scale):
        try:
            if isinstance(resize_scale, (int, float)) and resize_scale > 0:
                ...
                raise ValueError(
                    "Invalid resizeScale value. Expected a positive and non-zero int or float. resizeScale has been set to 1.")
    ...
    def optimize_frame(self, frame):
        try:
            ...
            return contrast_enhanced_frame
        except Exception as e:
            print(e)
            return None
    ...
    def __convert_to_grayscale(self, frame):
        try:
            ...
            if frame.shape[-1] != 3 or frame.ndim != 3:
                raise ValueError(
                    "Invalid frame format. Expected RGB color format.")
    ...
    def __enhance_contrast(self, frame):
        try:
            ...
            if len(frame.shape) != 2:
                raise ValueError(
                    "Invalid frame format. Expected grayscale image.")
    ...
    def __resize(self, frame):
        try:
            ...
            resized_frame = cv2.resize(
                frame, None, fx=self.__resize_scale, fy=self.__resize_scale)
